...
class Videos(object):
    ...
    def _folder_blobs(self, date_list, folder_name):
        ...
        for blob in blob_list:
            for each_date in date_list:
                if 'check' or '2012' in blob.name:
                    print(each_date)
                    folder_blobs.append(blob.name)
                    date_list.remove(each_date)
                    break
            if not date_list:
                print('the date is empty now')
                break
        return folder_blobs
    ...
    def blob_video(self):
        ...
        for blob in blob_list:
            blob_videos.append(blob_service.get_blob_url(container, blob.name))
        return blob_videos



   
s = Videos(date(2018,12,29), date.today())
print(s._blob_list())
